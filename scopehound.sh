...
# --- Step 2: Port Scan (Fixed Saving) ---
echo -e "${BLUE}[*] Running Port Scan (Top 100)...${NC}"
# Use -o explicitly and -connect-scan for reliability without sudo
...
# --- Step 4: GAU & GF Patterns ---
if [ "$ALIVE_COUNT" -gt 0 ]; then
    echo -e "${BLUE}[*] Fetching URLs & Processing GF Patterns...${NC}"
    cat "$RESULTS_DIR/alive_subs.txt" | gau --subs > "$RESULTS_DIR/gau.txt" 2>/dev/null
    
    mkdir -p "$RESULTS_DIR/gf_patterns"
    gf xss "$RESULTS_DIR/gau.txt" > "$RESULTS_DIR/gf_patterns/xss.txt" 2>/dev/null
    gf ssrf "$RESULTS_DIR/gau.txt" > "$RESULTS_DIR/gf_patterns/ssrf.txt" 2>/dev/null
    gf sqli "$RESULTS_DIR/gau.txt" > "$RESULTS_DIR/gf_patterns/sqli.txt" 2>/dev/null
    gf rce "$RESULTS_DIR/gau.txt" > "$RESULTS_DIR/gf_patterns/rce.txt" 2>/dev/null
...
# 1. Interesting Ports (Excluding standard 80/443)
echo -e "${GREEN}[+] Interesting Open Ports:${NC}"
if [ -s "$RESULTS_DIR/open_ports.txt" ]; then
    # Grep -v filters out lines ending in :80 or :443
    INTERESTING_PORTS=$(grep -vE ':80$|:443$' "$RESULTS_DIR/open_ports.txt")
    if [ -z "$INTERESTING_PORTS" ]; then
        echo -e "    ${RED}No unique ports found (Only 80/443).${NC}"
    else
        echo "$INTERESTING_PORTS" | sed 's/^/    - /'
    fi
else
    echo -e "    ${RED}No ports discovered.${NC}"
fi

echo -e ""

# 2. Nuclei Findings (Excluding [info])
...
echo -e "${YELLOW}===============================================${NC}"
echo -e "${GREEN}[+] Recon Completed. Results in: $RESULTS_DIR${NC}"
